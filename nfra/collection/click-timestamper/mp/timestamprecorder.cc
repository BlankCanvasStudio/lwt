...
// This needs to be an interprocess mutex
...
void write_data(int size, double stmp, String filename) {
    ...
    if (childPid == 0) {
        ...
        // Change the process name so when we kill click, its 
        //   definitely the parent process and not one of the hildren
        const char* newProcessName = "WriterProcess";

        if (prctl(PR_SET_NAME, reinterpret_cast<unsigned long>(newProcessName), 0, 0, 0) != 0) {
            std::cerr << "Error changing process name" << std::endl;
            exit(0);
        } 
        ...
        click_chatter("Failed to open file");
    }
}
...
DataRecorder::simple_action(Packet *p) {
    ...
    write_data(packet_size, timeInSeconds, _name);
