...
ThreadPool* DataRecorder::_pool = nullptr;
...
DataRecorder::simple_action(Packet *p) {
    ...
    _pool->enqueue(p->length(), p->timestamp_anno().doubleval());
...
String
DataRecorder::read_handler(Element *e, void* other) {
    
    _pool->dump();
    return String(_pool->index() + 1);
