...
gcc -Wall -Wextra -Werror -O3 -std=gnu17 server.c -o rcv.o
```
2. In C++
```bash
...
*/
    ...
    //
    retcode = inet_pton(AF_INET, "10.0.6.2", &addr_server.sin_addr); // <--- Option 3/3 to set the IP address
    if (retcode != 1)
    {
    ...
    while (1) {
        ...
        num_bytes_received = recvfrom(socket_fd, receive_buf, sizeof(receive_buf), MSG_WAITALL,
    ...
    strncpy(sender_ip_addr_str, sender_ip_addr, sizeof(sender_ip_addr_str)-1);
