...
gcc -Wall -Wextra -Werror -O3 -std=c17 client.c -o gen.o
```
2. In C++
```bash
...
#define MAX_RECEIVE_BUFFER_SIZE 1048576 // in bytes
...
void generateRandomBytes(int x, unsigned char* buffer) {
    for (int i = 0; i < x; i++) {
...
int main()
{
    ...
    addr_server.sin_addr.s_addr = inet_addr("10.0.6.2");
    ...
    char msg_to_send[PACKET_SIZE];

    while(1) {


        ssize_t num_bytes_sent = sendto(socket_fd, msg_to_send, PACKET_SIZE, 0,
