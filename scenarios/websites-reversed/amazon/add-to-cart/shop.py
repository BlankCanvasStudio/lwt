...
def add_to_cart():
    ...
    firefox.implicitly_wait(1)
    input("Waiting before adding to cart")
    btn = firefox.find_element(By.ID, "add-to-cart-button")
    btn.click()
    firefox.implicitly_wait(1)
...
for _ in range(0, num_items_to_check):
    ...
    # Its not a bug if you catch it
    if 'cart' in firefox.current_url:
        firefox.execute_script("window.history.go(-2)")
    else:
