...
import os
import time
import struct, traceback, numpy as np

def str_to_sinste(fname):
    fname = fname.split('/')[-2] # New indexing strategy
...
"""
    Data returned from the is [ [ time, size ] ]
"""
...
def read_data(path): #, length=5000):
    ...
    [ sizes, times ] = [ np.zeros(length, dtype=int), np.zeros(length, dtype=np.float64) ]
...
def load_cell(fname, time=0, ext=".csv"):
    #time = 0 means don't load packet times (saves time and memory)
    data = []
    starttime = -1
    try:
        if ext == '.csv':
            [ times, sizes ] = read_data(fname)
        ...
        if ext == ".htor":
            ...
            #not truly implemented. Actually dead wrong
            data = list(lines[0])            
