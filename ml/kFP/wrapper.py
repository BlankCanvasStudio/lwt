...
def get_pkt_list(fname):
    ...
    return np.array(list(zip(times, direction)))
...
def TOTAL_FEATURES(trace_data, max_size=175): # tracedata is now a filename but to lazy to copy and paste yet
    ...
    stdconc, avgconc, medconc, minconc, maxconc, conc = pkt_concentration_stats(Total) # 5 + x

    avg_per_sec, std_per_sec, med_per_sec, min_per_sec, max_per_sec, per_sec = number_per_sec(Total) # 6 long

    avg_order_in, avg_order_out, std_order_in, std_order_out = avg_pkt_ordering_stats(Total) # 4

    perc_in, perc_out = perc_inc_out(In, Out, Total) # 2 lomg
...
def in_mem_extract(ofname, max_class):
    ...
    testlabels = np.append(testlabels, trainlabels)
    testdata   = np.append(testdata,   traindata, axis=0)
