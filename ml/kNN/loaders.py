...
def load_options(fname):
    ...
    for line in open(fname, "r"):
        ...
        if ":" in line:
            li = [ x.strip() for x in line.split('#')[0].split(":") ] # Now we can proccess inline
...
def get_list(d):
    ...
    # This seems to be getting the # of close world examples. They use this to build name arrays
    if "CLOSED_SITESTART" in d.keys(): # Unsure of this difference
        cstart = d["CLOSED_SITESTART"]
        cend = d["CLOSED_SITEEND"]
    else:
        cstart = 0
    ...
    if "OPEN_INSTSTART" in d.keys(): # Unsure of this difference
        ostart = d["OPEN_INSTSTART"]
        oend = d["OPEN_INSTEND"]
    else:
        ostart = 0
    ...
    if "CLOSED_INSTSTART" in d.keys(): # Unsure of this difference
        istart = d["CLOSED_INSTSTART"]
        iend = d["CLOSED_INSTEND"]
    else:
        istart = 0
        iend = d["CLOSED_INSTNUM"]

    reqs = ["DATA_LOC", "FOLD_MODE", "FOLD_NUM", "DATA_TYPE"]
    for req in reqs:
    ...
    # Create the list of file names for training set
    train_list = []
    test_list = []
    ...
    # The following is what populates the naming arrays based on options file
    ...
    if (d["FOLD_MODE"] == 2):
        for s in range(cstart, cend):
            for i in range(istart, iend):
                sname = d["DATA_LOC"] + str(s) + "-" + str(i) + "/pipe_rcv." + d["DATA_TYPE"]
                train_list[s-cstart].append(sname)
                test_list[s-cstart].append(sname)
        for s in range(ostart, oend):
            sname = d["DATA_LOC"] + str(s) + "/pipe_rcv." + d["DATA_TYPE"]
    ...
    if (d["FOLD_MODE"] == 3):
        for s in range(cstart, cend):
            for i in range(istart, iend):
                di = i - istart
                sname = d["DATA_LOC"] + str(s) + "-" + str(i) + "/pipe_rcv." + d["DATA_TYPE"]
                if (di >= dnum/foldtotal * d["FOLD_NUM"] and
                    di < dnum/foldtotal * (d["FOLD_NUM"]+1)):
                    test_list[s-cstart].append(sname)
                else:
                    train_list[s-cstart].append(sname)
        for s in range(ostart, oend):
    ...
    if (d["FOLD_MODE"] == 4):
        ...
        for s in range(cstart, cend):
            for i in range(istart, iend):
                di = i - istart
                sname = d["DATA_LOC"] + str(s) + "-" + str(i) + "/pipe_rcv." + d["DATA_TYPE"]
                if (di >= dnum/foldtotal * trainfoldnum and
                    di < dnum/foldtotal * (trainfoldnum+1)):
                    train_list[s-cstart].append(sname)
        for s in range(ostart, oend):
            sname = d["DATA_LOC"] + str(s) + "/pipe_rcv." + d["DATA_TYPE"]
