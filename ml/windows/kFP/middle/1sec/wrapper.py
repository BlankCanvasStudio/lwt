...
def interarrival_maxminmeansd_stats(In, Out, Total):# (list_data):
    ...
    if len(In) and len(Out):
        avg_in = np.sum(In)/float(len(In))
        avg_out = np.sum(Out)/float(len(Out))
        avg_total = np.sum(Total)/float(len(Total))
        interstats.append((np.max(In), np.max(Out), np.max(Total), avg_in, avg_out, avg_total, np.std(In), np.std(Out), np.std(Total), np.percentile(In, 75), np.percentile(Out, 75), np.percentile(Total, 75)))
    elif len(Out) and not len(In):
        avg_out = np.sum(Out)/float(len(Out))
        avg_total = np.sum(Total)/float(len(Total))
        interstats.append((0, np.max(Out), np.max(Total), 0, avg_out, avg_total, 0, np.std(Out), np.std(Total), 0, np.percentile(Out, 75), np.percentile(Total, 75)))
    elif len(In) and not len(Out):
        avg_in = np.sum(In)/float(len(In))
        avg_total = np.sum(Total)/float(len(Total))
        interstats.append((np.max(In), 0, np.max(Total), avg_in, 0, avg_total, np.std(In), 0, np.std(Total), np.percentile(In, 75), 0, np.percentile(Total, 75)))
    else:
...
def TOTAL_FEATURES(trace_data, max_size=175): # tracedata is now a filename but to lazy to copy and paste yet
    ...
    # Duration is in sec
    Total = get_pkt_list(trace_data, 25, duration = 1)
    In, Out = In_Out(Total)
    ...
    stdconc, avgconc, medconc, minconc, maxconc, conc = pkt_concentration_stats(Total) # 27-32

    avg_per_sec, std_per_sec, med_per_sec, min_per_sec, max_per_sec, per_sec = number_per_sec(Total) # 33-38

    avg_order_in, avg_order_out, std_order_in, std_order_out = avg_pkt_ordering_stats(Total) # 39=43

    perc_in, perc_out = perc_inc_out(In, Out, Total) # 44-46
    ...
    ALL_FEATURES = np.append(ALL_FEATURES, thirtypkts)
    # log('thirtypkts:', running_list, '-', running_list + (len(thirtypkts) if hasattr(thirtypkts, '__len__') else 1) - 1)
    # running_list += len(thirtypkts) if hasattr(thirtypkts, '__len__') else 1
    ALL_FEATURES = np.append(ALL_FEATURES, stdconc)
    # log('stdconc:', running_list, '-', running_list + (len(stdconc) if hasattr(stdconc, '__len__') else 1) - 1)
    # running_list += len(stdconc) if hasattr(stdconc, '__len__') else 1
    ALL_FEATURES = np.append(ALL_FEATURES, avgconc)
    # log('avgconc:', running_list, '-', running_list + (len(avgconc) if hasattr(avgconc, '__len__') else 1) - 1)
    # running_list += len(avgconc) if hasattr(avgconc, '__len__') else 1
    ...
    ALL_FEATURES = np.append(ALL_FEATURES, avg_order_out)
    # log('avg_order_out:', running_list, '-', running_list + (len(avg_order_out) if hasattr(avg_order_out, '__len__') else 1) - 1)
    # running_list += len(avg_order_out) if hasattr(avg_order_out, '__len__') else 1
    ...
    ALL_FEATURES = np.append(ALL_FEATURES, std_order_out)
    # log('std_order_out:', running_list, '-', running_list + (len(std_order_out) if hasattr(std_order_out, '__len__') else 1) - 1)
    # running_list += len(std_order_out) if hasattr(std_order_out, '__len__') else 1
    ALL_FEATURES = np.append(ALL_FEATURES, medconc)
    # log('medconc:', running_list, '-', running_list + (len(medconc) if hasattr(medconc, '__len__') else 1) - 1)
    # running_list += len(medconc) if hasattr(medconc, '__len__') else 1
    ...
    ALL_FEATURES = np.append(ALL_FEATURES, maxconc)
    # log('maxconc:', running_list, '-', running_list + (len(maxconc) if hasattr(maxconc, '__len__') else 1) - 1)
    # running_list += len(maxconc) if hasattr(maxconc, '__len__') else 1
    ALL_FEATURES = np.append(ALL_FEATURES, perc_in)
    # log('perc_in:', running_list, '-', running_list + (len(perc_in) if hasattr(perc_in, '__len__') else 1) - 1)
    # running_list += len(perc_in) if hasattr(perc_in, '__len__') else 1
    ALL_FEATURES = np.append(ALL_FEATURES, perc_out)
    # log('perc_out:', running_list, '-', running_list + (len(perc_out) if hasattr(perc_out, '__len__') else 1) - 1)
    # running_list += len(perc_out) if hasattr(perc_out, '__len__') else 1
    ALL_FEATURES = np.append(ALL_FEATURES, altconc)
    # log('altconc:', running_list, '-', running_list + (len(altconc) if hasattr(altconc, '__len__') else 1) - 1)
    # running_list += len(altconc) if hasattr(altconc, '__len__') else 1
    ...
    ALL_FEATURES = np.append(ALL_FEATURES, sum(altconc))
    # running_list += 1
    # log('altconc sum:', running_list - 1)
...
def GenerateResults(guesses, answers, probs, forest, x_test):
    ...
    feature_names = [ f'feature_{x}' for x in range(175) ]
    class_names =[ f'class_{x}' for x in range(91) ]
    ...
    # save_tree_image(forest, feature_names, class_names, "tree" + str(i) + '.png')

    # predict_fn_f = lambda x: forest.predict_proba(x_test).astype(float)
    # explainer = lime.lime_tabular.LimeTabularExplainer(x_test, mode='classification', class_names = range(num_classes) )

    # print(len(x_test[5]))

    # x_test_reshaped = np.array(x_test).reshape(1, -1)

    # exp = explainer.explain_instance(x_test_reshaped[0], predict_fn_f, num_features = len(x_test[5]), num_samples = 1)

    # PREDICTION_FILE = d["OUTPUT_LOC"] + '/' + 'c' + str(d["CLOSED_SITENUM"]) + '-i' + str(d["CLOSED_INSTNUM"]) + '-o' + str(d["OPEN_SITENUM"]) + '-i' + str(d["OPEN_INSTNUM"]) + '/' + 'preds.html'
    # exp.save_to_file(PREDICTION_FILE)

    log('\n')



def save_tree_image(tree, feature_names, class_names, filename):
